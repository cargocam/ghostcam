...
func (db *PostgresDB) InsertSegments(ctx context.Context, segments []SegmentRecord) error {
	...
	for _, s := range segments {
		batch.Queue(
			...
			s.SegmentID, s.DeviceID, s.S3Key, int64(s.StartTS), int64(s.EndTS), int64(s.SizeBytes), s.Resolution, int64(s.CreatedAt))
	}
...
func (db *PostgresDB) ListSegments(ctx context.Context, deviceID string, fromTS, toTS uint64) ([]SegmentRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.SegmentID, &s.DeviceID, &s.S3Key, &startTS, &endTS, &sizeBytes, &s.Resolution, &createdAt); err != nil {
...
func (db *PostgresDB) LatestSegment(ctx context.Context, deviceID string) (*SegmentRecord, error) {
	...
	var s SegmentRecord
	var startTS, endTS, sizeBytes, createdAt int64
	err := row.Scan(&s.SegmentID, &s.DeviceID, &s.S3Key, &startTS, &endTS, &sizeBytes, &s.Resolution, &createdAt)
