...
|----------|---------|-------------|
| `GHOSTCAM_DATA_DIR` | `/var/ghostcam` | Data directory: CA keys, SQLite DB, server cert |
...
On first start the server creates `GHOSTCAM_DATA_DIR`, generates a CA keypair, self-signs a server TLS certificate, initialises the SQLite database, and prints an initial password for the `admin` account.
...
| `db` | `SqliteDatabase` — SQLite implementation of `Database` via `sqlx` |
