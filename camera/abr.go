...
// second (matches rpicam-vid --bitrate units).
type ABRTier struct {
	Name    string
	Width   uint32
	Height  uint32
...
// yet decided). capture.go reads this at runRealPipeline entry; ABR
// writes it as part of every tier shift.
...
// the package-level currentPublisher atomic so a publisher reconnect
// is observed transparently.
type ABRController struct {
	...
	getPublisher  func() *Publisher
	sampleFn      func() (VideoSendSample, bool)
	now           func() time.Time
	signalRestart func()

	// Per-tick stat carry. Tracks the previous cumulative sample so
	// the controller can diff to a delta.
	lastSample VideoSendSample
	haveLast   bool
	lastPub    *Publisher // detect publisher swap and reset diffs
}
...
func NewABRController(start ABRTier) *ABRController {
	c := &ABRController{
		...
		getPublisher:  func() *Publisher { return currentPublisher.Load() },
		now:           time.Now,
		signalRestart: func() { requestPipelineRestart.Store(true) },
	}
...
func indexOfTier(tiers []ABRTier, name string) int {
	for i, t := range tiers {
		if t.Name == name {
			return i
		}
