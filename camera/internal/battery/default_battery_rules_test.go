...
func TestDefaultBatteryRules_ShapeAndOrdering(t *testing.T) {
	...
	if rules[0].Threshold != 10 || rules[0].PowerMode != state.PowerModeSleep || rules[0].UploadMode != "lazy" {
		t.Errorf("unexpected rule 0: %+v", rules[0])
	}
	if rules[1].Threshold != 20 || rules[1].PowerMode != state.PowerModeStandby || rules[1].UploadMode != "lazy" {
...
func TestDefaultBatteryRules_EvaluatesCorrectly(t *testing.T) {
	...
	cases := []struct {
		...
		{50, ""},
		{21, ""},
		{20, state.PowerModeStandby},
		{15, state.PowerModeStandby},
		{11, state.PowerModeStandby},
		{10, state.PowerModeSleep},
		{5, state.PowerModeSleep},
		{0, state.PowerModeSleep},
	}
