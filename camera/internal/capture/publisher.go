...
// VideoSendSample is the canonical shape capture publishes for the ABR
// controller; it lives in internal/state so both capture and abr can
// reference it without a cycle. The local alias keeps existing call
// sites in this file unchanged.
type VideoSendSample = state.VideoSendSample
