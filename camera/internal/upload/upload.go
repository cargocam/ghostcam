...
// Client is the upload loop's narrow view of the server HTTP client.
// The concrete *main.Client (camera/client.go) satisfies this surface;
// defining the interface here lets upload live in its own subpackage
// without importing package main. UploadFile is expected to return an
// *S3UploadError (errors.As-detectable) when S3 returns 4xx, signalling
// the loop to discard cached presigned URLs.
type Client interface {
	RequestPresignedURLs(ctx context.Context, count uint32, uploaded []common.UploadedSegment) (*common.PresignResponse, error)
	UploadFile(ctx context.Context, presignedURL string, data []byte) error
	UploadInit(ctx context.Context, data []byte) error
}

// S3UploadError is returned by Client.UploadFile when S3 returns a
// non-2xx status. Lives in upload (rather than alongside the rest of
// the HTTP client) so the upload loop's retry path can check for it
