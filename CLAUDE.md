...
**Verifying cellular actually carries traffic** (`force_cellular` command / `camera/internal/uplink`): a dev control that takes WiFi down for N seconds so the camera is forced onto its cellular bearer, then auto-restores WiFi. The revert is a *persisted deadline* (`{dataDir}/force_cellular_until`) enforced by a watchdog that re-checks every few seconds and at startup — so if the daemon restarts (or the Pi reboots) mid-force, WiFi is restored the moment the deadline passes, even if that happened while the daemon was down. A cellular link that never comes up therefore can't permanently black-hole a remote camera. Duration is clamped to `ForceCellularMaxSeconds` (30 min); `seconds<=0` reverts immediately. Push via `POST /api/v1/admin/cameras/{deviceID}/force_cellular`. Watch `uplink_iface` in telemetry flip to `wwan0` to confirm.
