...
**Coarse location (cell-tower GPS fallback)**: when there's no GPS fix, the camera reports its serving-cell 3GPP identifiers (`cell_op`=MCC+MNC, `cell_lac`, `cell_tac`, `cell_cid`) read from `mmcli -m 0 --location-get` (unprivileged; 3GPP location is enabled by the GPS oneshot). The **server** resolves those to a coarse lat/lon via a geolocation provider (OpenCellID) and stamps `location_source="cell"` + `location_accuracy_m` on the stored telemetry so the map shows an approximate point when GPS is unavailable. Camera side is `sensors.ReadCellLocation`; it only reports when `d.Lat == nil` (no GPS), so it costs nothing when real GPS is present. Resolution is server-side + key-gated (no-op without `OPENCELLID_API_KEY`) — see the server repo's `geolocation` package.

  Two pieces make the cell read work, both shipped in the image/`.deb` (zero-SSH — the postinst + GPS oneshot handle them, no user setup): (1) `ghostcam-enable-gps.sh` enables the 3GPP source alongside GPS (`mmcli --location-enable-gps-nmea --location-enable-gps-raw --location-enable-3gpp`) every boot, since ModemManager doesn't persist location settings; (2) `pi/debian/polkit/49-ghostcam-mm.rules` grants the `netdev` group the `org.freedesktop.ModemManager1.Location` action so the non-root daemon's `mmcli --location-get` isn't polkit-rejected (scoped to Location only — deliberately not Messaging/Voice, so a camera can't send SMS/place calls on the SIM). **OTA caveat:** both are system files, so like the GPS oneshot they reach the field only via reflash/apt, not a binary-only firmware OTA.

**Verifying cellular actually carries traffic** (`force_cellular` command / `camera/internal/uplink`): a dev control that takes WiFi down for N seconds so the camera is forced onto its cellular bearer, then auto-restores WiFi. **Fail-open by design**: the force deadline is *in-memory only* (never persisted), and the watchdog's first action on startup is to re-enable WiFi + delete any stale legacy marker — so a daemon restart or reboot **always** brings WiFi back. This is deliberate: an earlier persisted-deadline version re-applied "WiFi off" on reboot and nearly stranded a field unit off-network. The trade-off (a crash mid-force ends the force early) is the safe direction. Duration is clamped to `ForceCellularMaxSeconds` (30 min); `seconds<=0` reverts immediately. Push via `POST /api/v1/admin/cameras/{deviceID}/force_cellular`. Watch `uplink_iface` in telemetry flip to `wwan0` to confirm.
