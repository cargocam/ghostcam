...
- **Billing always on**: Every user defaults to the "free" tier (5 GB, 1 camera). `effectiveTier()` derives tier from Stripe subscription state; when Stripe not configured (dev), returns "enterprise" (unlimited). In local dev, set `STRIPE_SECRET_KEY` in `.env` to enable real tier enforcement.
